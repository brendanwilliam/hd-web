...
import { inputScatterPoints, reportSeriesGroups } from "@/features/reports/visualizations/data";
...
export function ChartPanel({ payload, group, duration, onHoverTime, previewTime = 0, onScrubTime }: { payload: ReportData; group: VisualizationGroup; duration: number; onHoverTime: (time: number | null) => void; previewTime?: number; onScrubTime?: (time: number) => void }) {
  ...
  return <section className="report-panel report-chart-panel">
    ...
    {inputPanel ? inputPoints.length ? <InputScatterPlot points={inputPoints} previewTime={previewTime} onHoverTime={onHoverTime} onScrubTime={onScrubTime ?? (() => {})} /> : <p className="report-note">Input samples with both Actions and Mouse distance are unavailable for this report.</p> : active.length ? <ComparisonChart active={active} mode={mode} duration={duration} label={definition.label} onHoverTime={onHoverTime} /> : <p className="report-note">Select a series to compare recorded data.</p>}
