...
export function InputScatterPlot({ points, onHoverTime }: { points: InputScatterPoint[]; onHoverTime: (time: number | null) => void }) {
  ...
  const togglePlayback = () => { if (frame >= points.length - 1) setFrame(0); setPlaying(previous => !previous || frame >= points.length - 1); };
  const trailPath = d3.line<InputScatterPoint>().x(point => x(point.apm)).y(point => y(point.velocityCms))(trail) ?? "";

  return <div className="report-chart input-scatter-chart">
    <div className="input-animation-controls"><button type="button" onClick={togglePlayback} aria-pressed={playing}>{playing ? "Pause sequence" : frame >= points.length - 1 ? "Replay sequence" : "Play sequence"}</button><span>{trail.length.toLocaleString()} / {points.length.toLocaleString()} samples</span></div>
    <svg ref={plotRef} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Input sample scatterplot with 3-second rolling Actions per minute on the horizontal axis, mouse velocity in centimeters per second on the vertical axis, and marginal distribution histograms">
      ...
      {trail.length > 1 && <path className="input-scatter-trail" d={trailPath} />}
