...
export function InputScatterPlot({ points, onHoverTime, onPreviewTime }: { points: InputScatterPoint[]; onHoverTime: (time: number | null) => void; onPreviewTime: (time: number | null) => void }) {
  const plotRef = useRef<SVGSVGElement>(null);
  const [tooltip, setTooltip] = useState<{ x: number; y: number; point: InputScatterPoint } | null>(null);
  const [playing, setPlaying] = useState(false), [frame, setFrame] = useState(0), [previewing, setPreviewing] = useState(false);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setFrame(previous => {
      if (previous >= points.length - 1) { setPlaying(false); return previous; }
      return previous + 1;
    }), 75);
    return () => window.clearInterval(timer);
  }, [playing, points.length]);

  useEffect(() => { if (previewing) onPreviewTime(trail.at(-1)?.seconds ?? null); }, [onPreviewTime, previewing, trail]);
  ...
  const showTooltip = (event: PointerEvent<SVGCircleElement> | FocusEvent<SVGCircleElement>, point: InputScatterPoint) => {
    ...
    onHoverTime(point.seconds);
  };
  const togglePlayback = () => { if (frame >= points.length - 1) setFrame(0); setPreviewing(true); setPlaying(previous => !previous || frame >= points.length - 1); };

  return <div className="report-chart input-scatter-chart">
    <div className="input-animation-controls"><button type="button" onClick={togglePlayback} aria-pressed={playing}>{playing ? "Pause sequence" : frame >= points.length - 1 ? "Replay sequence" : "Play sequence"}</button><span>Game time: {formatTime(trail.at(-1)?.seconds ?? 0)} / {formatTime(points.at(-1)?.seconds ?? 0)}</span></div>
    <svg ref={plotRef} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Input sample scatterplot with 3-second rolling Actions per minute on the horizontal axis, mouse velocity in centimeters per second on the vertical axis, and marginal distribution histograms">
      ...
      {points.map((point, index) => <circle key={index} className="input-scatter-point" cx={x(point.apm)} cy={y(point.velocityCms)} r="2" tabIndex={0} aria-label={`${formatTime(point.seconds)}: ${point.apm.toFixed(1)} APM, ${point.velocityCms.toFixed(2)} cm/s`} onPointerEnter={event => showTooltip(event, point)} onPointerLeave={() => { setTooltip(null); onHoverTime(null); }} onFocus={event => showTooltip(event, point)} onBlur={() => { setTooltip(null); onHoverTime(null); }} />)}
