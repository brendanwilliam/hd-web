...
export async function fetchRecentMatchesAction(formData: FormData) {
  ...
  if (separator < 1 || !gameName || !tagLine)
    redirect("/reports?fetch_error=invalid_riot_id");

  let imported: number;
  try {
    imported = await importRecentMatches(account.id, gameName, tagLine);
  } catch (error) {
    const code =
