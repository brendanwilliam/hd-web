...
export async function reconcileReport(reportId: string) {
  ...
  if (!report || ["matched", "identity_not_found", "ambiguous_match", "needs_attention"].includes(report.reconciliationState)) return;
  try {
    ...
    if (!account || !region || !text(account.puuid)) return mark(reportId, "identity_not_found", "Riot ID was not found");
    ...
    if (candidates.length > 1) return mark(reportId, "ambiguous_match", "Multiple plausible matches");
...
async function mark(id: string, state: "input_only" | "identity_not_found" | "ambiguous_match", message: string) {
