...
import { hydrateRiotMatchTimeline } from "./hydrate-riot-match-timeline";
...
export async function importRecentMatches(
  ...
  for (const candidate of regions) {
    account = data(await riot(accountByRiotIdUrl(candidate, gameName, tagLine)));
  ...
  if (!account || !region || !text(account.puuid)) {
    throw new Error("riot_id_not_found");
  }
  ...
  let imported = 0;
  for (const matchId of ids.filter(
    (value): value is string => typeof value === "string",
  )) {
    const match = data(await riot(matchUrl(region, matchId)));
...
function matchData(
  ...
  return {
    accountId,
    matchId,
    riotGameId: number(info.gameId) ? String(number(info.gameId)) : null,
    riotIdGameName: gameName,
    riotIdTagLine: tagLine,
    riotRegion: region,
    gameStartedAt: new Date(number(info.gameStartTimestamp)),
    gameMode: text(info.gameMode) || "UNKNOWN",
