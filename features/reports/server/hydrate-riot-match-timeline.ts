...
export async function hydrateRiotMatchTimeline(accountId: string, riotMatchId: string) {
  const match = await db.riotMatch.findFirst({
    where: { id: riotMatchId, accountId },
  });
  if (!match || match.timelineState === "ready") return match;
  try {
    const timeline = await requestTimeline(match.riotRegion, match.matchId);
    return db.riotMatch.update({
      where: { id: match.id },
      data:
        timeline === null
          ? {
              timelineState: "unavailable",
              timelineError: "riot_timeline_not_found",
              riotEvents: Prisma.JsonNull,
            }
          : {
              timelineState: "ready",
              timelineError: null,
    ...
    return db.riotMatch.update({
      ...
      data: {
        ...
        timelineError:
          error instanceof Error ? error.message : "riot_timeline_fetch_failed",
      },
    });
  }
}
