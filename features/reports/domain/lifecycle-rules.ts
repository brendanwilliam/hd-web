...
export type ObjectiveLifecycle = {
  kind: ObjectiveKind;
  subtype: DragonElement | null;
  gameOrder: number | null;
  expectedSpawnAtMs: number | null;
  observedSpawnAtMs: number | null;
  slainAtMs: number | null;
  slainByTeamId: number | null;
  respawnAtMs: number | null;
  despawnAtMs: number | null;
  endReason: "slain" | "despawned" | "replaced" | "game_end" | "unknown" | null;
  applicableRulePatch: string;
...
export type PlayerDeathInterval = {
  ...
  teamId: number;
  diedAtMs: number;
  predictedRespawnAtMs: number | null;
...
export type TeamDragonState = {
  ...
  establishedRiftElement: DragonElement | null;
  elderAspect: {
    grantedAtMs: number | null;
    expiresAtMs: number | null;
    recipientParticipantIds: number[];
    availability: Availability;
  };
};

export type ObjectiveRule = {
  initialSpawnAtMs: number | null;
  respawnAfterMs: number | null;
  despawnAtMs: number | null;
  despawnInCombatAtMs: number | null;
...
export type LifecycleRules = {
  ...
  sourcePatch: string;
  championBaseRespawnMs: number[];
  objectives: Record<ObjectiveKind, ObjectiveRule>;
  dragonSequence: {
...
const currentObjectives: Record<ObjectiveKind, ObjectiveRule> = {
  elemental_dragon: {
    initialSpawnAtMs: 5 * minute,
    respawnAfterMs: 5 * minute,
    despawnAtMs: null,
    despawnInCombatAtMs: null,
    replacement: "elder_after_dragon_soul",
  },
  elder_dragon: {
    initialSpawnAtMs: null,
    respawnAfterMs: 6 * minute,
    despawnAtMs: null,
    despawnInCombatAtMs: null,
    replacement: "none",
  },
  baron_nashor: {
    initialSpawnAtMs: 20 * minute,
    respawnAfterMs: 6 * minute,
    despawnAtMs: null,
    despawnInCombatAtMs: null,
    replacement: "none",
  ...
  rift_herald: {
    initialSpawnAtMs: 15 * minute,
    respawnAfterMs: null,
    despawnAtMs: 19 * minute + 45_000,
    despawnInCombatAtMs: 19 * minute + 55_000,
    replacement: "none",
  },
  voidgrubs: {
    initialSpawnAtMs: 8 * minute,
    respawnAfterMs: null,
    despawnAtMs: 14 * minute + 45_000,
    despawnInCombatAtMs: 14 * minute + 55_000,
    replacement: "none",
  },
};

const pre1416RespawnMs = [
  6_000, 6_000, 8_000, 8_000, 10_000, 12_000, 16_000, 21_000, 26_000, 32_500, 35_000,
  37_500, 40_000, 42_500, 45_000, 47_500, 50_000, 52_500,
];
const currentRespawnMs = [
  10_000, 10_000, 12_000, 12_000, 14_000, 16_000, 20_000, 25_000, 28_000, 32_500, 35_000,
  37_500, 40_000, 42_500, 45_000, 47_500, 50_000, 52_500,
];

export const LIFECYCLE_RULESETS: LifecycleRules[] = [
  {
    effectiveFromPatch: "0.0.0",
    sourcePatch: "14.15.0",
    championBaseRespawnMs: pre1416RespawnMs,
    objectives: currentObjectives,
    dragonSequence: {
      initialDistinctCount: 3,
      soulStackCount: 4,
      elements: ["chemtech", "cloud", "hextech", "infernal", "mountain", "ocean"],
    },
    elderAspectDurationMs: 150_000,
  },
  {
    effectiveFromPatch: "14.16.0",
    sourcePatch: "26.15.0",
    championBaseRespawnMs: currentRespawnMs,
    objectives: currentObjectives,
    dragonSequence: {
      initialDistinctCount: 3,
      soulStackCount: 4,
...
export function selectLifecycleRules(gameVersion: string, rulesets = LIFECYCLE_RULESETS) {
  ...
  if (!patch)
    return {
      ...
      rules: null,
      applicableRulePatch: null,
    };
  const selected = [...rulesets]
    .sort((left, right) =>
      compareGamePatches(
        parseRequiredPatch(left.effectiveFromPatch),
        parseRequiredPatch(right.effectiveFromPatch),
      ),
    )
    .filter(
      (rules) =>
        compareGamePatches(parseRequiredPatch(rules.effectiveFromPatch), patch) <= 0,
    )
    .at(-1);
  return selected
    ? {
        ...
        rules: selected,
        applicableRulePatch: selected.effectiveFromPatch,
      }
    : { availability: "unknown" as const, rules: null, applicableRulePatch: null };
}

export function predictedRespawnAtMs(
  diedAtMs: number,
  level: number,
  rules: LifecycleRules,
) {
  if (!Number.isInteger(level) || level < 1 || level > rules.championBaseRespawnMs.length)
    return null;
