...
describe("v2 report payload", () => {
  ...
  it("accepts a Practice Tool capture", () => {
    const value = report(); value.capture.game_mode = "PRACTICETOOL";
    value.payload_hash = createHash("sha256").update(canonicalPayload(value as never)).digest("hex");
    expect(validateReport(value).success).toBe(true);
  });
