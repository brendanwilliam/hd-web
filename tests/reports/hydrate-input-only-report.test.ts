...
describe("hydrateInputOnlyReport", () => {
  ...
  it("reconciles an input-only report and returns its refreshed data", async () => {
    mocks.findReport
      .mockResolvedValueOnce({ id: "report-1", reconciliationState: "input_only" })
      .mockResolvedValueOnce({ id: "report-1", reconciliationState: "matched" });

    await expect(hydrateInputOnlyReport("account-1", "report-1")).resolves.toEqual({
      id: "report-1",
      reconciliationState: "matched",
    });
    expect(mocks.reconcile).toHaveBeenCalledWith("report-1");
    expect(mocks.findReport).toHaveBeenCalledTimes(2);
    expect(mocks.findReport).toHaveBeenNthCalledWith(1, {
  ...
  it("retries a pending report only after its backoff window", async () => {
    mocks.findReport
      ...
      .mockResolvedValueOnce({ id: "report-1", reconciliationState: "pending" });
