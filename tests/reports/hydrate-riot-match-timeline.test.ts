...
vi.mock("@/shared/server/db", () => ({
  db: { riotMatch: { findFirst: mocks.findMatch, update: mocks.updateMatch } },
}));
...
describe("hydrateRiotMatchTimeline", () => {
  ...
  it("retries a pending timeline on refresh and persists normalized events", async () => {
    ...
    await expect(
      hydrateRiotMatchTimeline("account-1", "riot-match-1"),
    ).resolves.toMatchObject({ timelineState: "ready" });
    const update = mocks.updateMatch.mock.calls[0][0];
    expect(update).toMatchObject({
      where: { id: "riot-match-1" },
      data: { timelineState: "ready", timelineError: null },
    });
    expect(update.data.riotEvents).toEqual([
  ...
  it("keeps the match available when the timeline fails", async () => {
    ...
    expect(mocks.updateMatch).toHaveBeenCalledWith({
      where: { id: "riot-match-1" },
      data: { timelineState: "pending", timelineError: "riot_429" },
    });
  });
});
