...
import { LIFECYCLE_RULESETS, compareGamePatches, parseGamePatch, predictedRespawnAtMs, respawnTimeIncrease, selectLifecycleRules } from "@/features/reports/domain/lifecycle-rules";

describe("timeline lifecycle rules", () => {
  it("compares Riot versions numerically and rejects malformed versions", () => {
    expect(compareGamePatches(parseGamePatch("14.10.0")!, parseGamePatch("14.9.9")!)).toBeGreaterThan(0);
  ...
  it("exposes the documented objective timings and Elder replacement", () => {
    ...
    expect(rules.objectives.elemental_dragon).toMatchObject({ initialSpawnAtMs: 300_000, respawnAfterMs: 300_000, replacement: "elder_after_dragon_soul" });
    expect(rules.objectives.elder_dragon.respawnAfterMs).toBe(360_000);
    expect(rules.objectives.voidgrubs).toMatchObject({ initialSpawnAtMs: 480_000, despawnAtMs: 885_000, despawnInCombatAtMs: 895_000 });
    expect(rules.dragonSequence).toMatchObject({ initialDistinctCount: 3, soulStackCount: 4 });
    expect(rules.elderAspectDurationMs).toBe(150_000);
  });

  it("returns rule-predicted respawns from the selected patch and applies late-game scaling", () => {
