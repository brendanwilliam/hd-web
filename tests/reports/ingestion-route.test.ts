...
const mocks = vi.hoisted(() => ({
  findToken: vi.fn(), findReport: vi.fn(), createReport: vi.fn(), updateToken: vi.fn(), reconcile: vi.fn(),
}));
...
describe("v2 report ingestion", () => {
  it("requires a valid device bearer token", async () => {
    expect((await POST(new Request("http://test/api/reports", { method: "POST" }))).status).toBe(401);
  ...
  it("rejects a non-canonical payload hash before persistence", async () => {
    const value = report(); value.payload_hash = "0".repeat(64);
  ...
  it("returns duplicate only for the same account payload and conflicts otherwise", async () => {
    const value = report();
    mocks.findReport.mockResolvedValue({ id: value.report_id, payloadHash: value.payload_hash });
    expect((await POST(request(value))).status).toBe(200);
    mocks.findReport.mockResolvedValue({ id: value.report_id, payloadHash: "f".repeat(64) });
