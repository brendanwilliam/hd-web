...
export async function POST(request: Request) {
  ...
  const [grants, tokens, reports] = await db.$transaction([
    ...
    db.report.findMany({ where: { reconciliationState: { in: ["pending", "input_only"] }, OR: [{ retryAt: null }, { retryAt: { lte: now } }] }, select: { id: true }, take: 50 })
  ]);
  await Promise.all(reports.map(report => reconcileReport(report.id)));
  return NextResponse.json({ expired_grants: grants.count, expired_tokens: tokens.count, reconciled_reports: reports.length });
}
