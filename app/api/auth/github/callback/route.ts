...
export async function GET(request: Request) {
  ...
  if (!code || !process.env.GITHUB_CLIENT_ID || !process.env.GITHUB_CLIENT_SECRET)
    return new NextResponse("GitHub sign-in failed", { status: 400 });
  ...
  const profileResponse = await fetch("https://api.github.com/user", {
    headers: { Authorization: `Bearer ${accessToken}`, "User-Agent": "hd-web" },
  });
  ...
  if (!profile.id || !profile.login)
    return new NextResponse("GitHub profile unavailable", { status: 401 });
