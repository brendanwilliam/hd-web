...
export async function POST(request: Request) {
  try {
    ...
    const grant = await db.deviceGrant.findUnique({
      where: { deviceCodeHash: digest(body.device_code) },
    });
    if (!grant || grant.expiresAt < new Date())
      return jsonError("expired device code", 410);
    if (!grant.approvedAt || !grant.accountId)
      return jsonError("authorization_pending", 428);
    if (grant.consumedAt) return jsonError("device code already consumed", 409);
    const token = secret(40);
    await db.$transaction([
      db.apiToken.create({
        data: { tokenHash: digest(token), accountId: grant.accountId, grantId: grant.id },
      }),
      db.deviceGrant.update({
        where: { id: grant.id },
