...
export async function GET(request: Request) {
  ...
  if (!code || !/^[A-F0-9]{4}-[A-F0-9]{4}$/.test(code))
    return jsonError("invalid device code");
  const grant = await db.deviceGrant.findFirst({
    where: { userCode: code, accountId: account.id },
  });
