...
export async function POST(request: Request) {
  ...
  const token = await db.apiToken.findFirst({
    where: { tokenHash: digest(bearer), revokedAt: null, scope: "reports:write" },
  });
  ...
  try {
    ...
    if (digest(canonicalPayload(report)) !== report.payload_hash)
      return jsonError("payload hash does not match report");
    const existing = await db.report.findUnique({
      where: { accountId_id: { accountId: token.accountId, id: report.report_id } },
    });
    if (existing) {
      if (existing.payloadHash !== report.payload_hash)
        return jsonError(
          "report ID was previously submitted with a different payload",
          409,
        );
      return NextResponse.json({
        id: existing.id,
        url: reportPath(existing.id),
        status: "duplicate",
      });
    }
    await db.report.create({
      data: {
        ...
        riotIdGameName: report.capture.riot_id.game_name,
        riotIdTagLine: report.capture.riot_id.tag_line,
        observedStartedAt: new Date(report.capture.started_at_utc),
    ...
    return NextResponse.json({
      id: report.report_id,
      url: reportPath(report.report_id),
      status: "accepted",
    });
  } catch (error) {
    return jsonError(
