...
import { attachRiotMatch, refreshRiotMatch } from "@/features/reports/server/attach-riot-match";
...
export default async function ReportPage({ params }: { params: Promise<{ riotId: string; reportId: string }> }) {
  ...
  return <main className="report-page"><a href={profilePath(profile.slug)}>← {profile.riotId}</a><section className="report-hero"><div className="report-hero-title"><p className="eyebrow">HANDS DIFF · MATCH GRAPHS</p><h1>{text(payload.champion)} match report</h1><p>{text(payload.game_mode)} · {Math.round(number(payload.duration_seconds) / 60)} minutes · {new Date(report.completedAt).toLocaleString()}</p></div><div className="report-summary"><div className="report-metrics"><span><b>{actions.toLocaleString()}</b> actions</span><span><b>{Math.round(distance).toLocaleString()} px</b> mouse distance</span><span><b>{number(payload.team_kills)} / {number(payload.enemy_team_kills)}</b> team kills</span></div><div className="report-hero-heatmap"><p className="eyebrow">MOUSE DWELL</p><MouseDwellHeatmap payload={payload} compact /></div></div>{account?.id === profile.accountId && gameId && <form className="report-refresh" action={refreshRiotMatch}><input type="hidden" name="profileId" value={profile.id} /><input type="hidden" name="reportId" value={report.id} /><button type="submit" aria-label="Refresh Riot match data" title="Refresh Riot match data">↻</button></form>}</section>{unresolved && <section className="report-panel"><h2>Post-game data unavailable</h2><p className="report-note">{lookup.status === "ambiguous" ? "More than one Riot match matched this report. Attach the correct match manually." : "Hands Diff could not safely identify this match from Riot match history yet."}</p>{account?.id === profile.accountId && <form action={attachRiotMatch}><input type="hidden" name="profileId" value={profile.id} /><input type="hidden" name="reportId" value={report.id} /><label>Region <select name="region" defaultValue="americas"><option value="americas">Americas</option><option value="europe">Europe</option><option value="asia">Asia</option><option value="sea">Sea</option></select></label><label>Riot game ID <input name="gameId" required placeholder="NA1_123456789" /></label><button type="submit">Attach post-game data</button></form>}</section>}<ReportVisualizations payload={payload} /></main>;
}
