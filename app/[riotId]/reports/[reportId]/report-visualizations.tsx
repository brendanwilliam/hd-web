...
function eventGroupTooltip(events: { event: Data; kind: string; seconds: number }[]) {
  if (events.length === 1) return eventTooltip(events[0].event, events[0].seconds, events[0].kind);
  const times = events.map(event => event.seconds);
  return `${events.length} events · ${formatTime(Math.min(...times))}–${formatTime(Math.max(...times))}\n${events.map(event => eventTooltip(event.event, event.seconds, event.kind)).join("\n")}`;
}
...
export function ReportVisualizations({ payload }: { payload: Data }) {
  ...
  const width = 800, height = 340, margin = { top: 20, right: 24, bottom: 0, left: 190 };
  ...
  return <>
    <section className="report-panel"><div className="report-controls"><label>View <select value={mode} onChange={event => setMode(event.target.value as Mode)}><option value="cumulative">Cumulative</option><option value="rate">Rate</option><option value="acceleration">Acceleration</option></select></label></div><div className="report-legend">{series.map(item => <button key={item.key} className={enabled.has(item.key) || !enabled.size ? "active" : ""} onClick={() => setEnabled(previous => { const next = new Set(previous.size ? previous : series.map(value => value.key)); next.has(item.key) ? next.delete(item.key) : next.add(item.key); return next; })}><i style={{ background: item.color }} />{item.label}</button>)}</div>{active.length ? <div className="report-chart"><svg ref={chartRef} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Independently normalized match comparison chart" onPointerMove={event => { const bounds = event.currentTarget.getBoundingClientRect(); setTooltip({ x: event.clientX - bounds.left, time: x.invert((event.clientX - bounds.left) / bounds.width * width) }); }} onPointerLeave={() => setTooltip(null)}>{d3.ticks(0, 100, 5).map(value => <g key={value}><line className="chart-grid" x1={margin.left} x2={width - margin.right} y1={y(value)} y2={y(value)} /><text className="chart-label" x={margin.left - 7} y={y(value) + 4} textAnchor="end">{value}%</text></g>)}{active.map(item => <path key={item.key} className="chart-series" stroke={item.color} d={d3.line<{ x: number; normalized: number }>().x(point => x(point.x)).y(point => y(point.normalized)).curve(item.step && mode === "cumulative" ? d3.curveStepAfter : d3.curveLinear)(normalized(item)) ?? ""} />)}{mode === "cumulative" && active.filter(item => item.key === "gold_spent").flatMap(item => normalized(item)).map((point, index) => <circle key={index} className="purchase-dot" cx={x(point.x)} cy={y(point.normalized)} r="3" />)}{tooltip && <line className="chart-cursor" x1={x(tooltip.time)} x2={x(tooltip.time)} y1={margin.top} y2={height} />}</svg>{tooltip && <aside className="chart-tooltip" style={{ left: `${Math.min(76, Math.max(1, tooltip.x / (chartRef.current?.clientWidth || width) * 100))}%` }}><b>{formatTime(tooltip.time)}</b>{active.map(item => { const point = nearest(item, tooltip.time); return <span key={item.key}><i style={{ background: item.color }} />{item.label}: {point.y.toLocaleString(undefined, { maximumFractionDigits: 2 })}{item.unit} <small>{formatTime(point.x)}</small></span>; })}</aside>}</div> : <p className="report-note">Select a series to compare recorded data.</p>}<div className="event-timeline"><svg viewBox={`0 0 ${width} ${timelineHeight}`} role="img" aria-label="Match events aligned to the comparison chart game-time axis">{strips.map(([kind, label], row) => <g key={kind}><text className="chart-label" x={margin.left - 7} y={row * 33 + 18} textAnchor="end">{label}</text><line className="chart-grid" x1={margin.left} x2={width - margin.right} y1={row * 33 + 14} y2={row * 33 + 14} />{(() => { const grouped = eventItems.filter(item => item.kind === kind).sort((first, second) => first.seconds - second.seconds).reduce<{ seconds: number; events: typeof eventItems }[]>((groups, item) => { const last = groups[groups.length - 1]; if (last && x(item.seconds) - x(last.events[last.events.length - 1].seconds) <= 12) { last.events.push(item); last.seconds = d3.mean(last.events, event => event.seconds) ?? item.seconds; } else groups.push({ seconds: item.seconds, events: [item] }); return groups; }, []); return grouped.map((group, index) => <g key={index} className="event-group" tabIndex={0}><title>{eventGroupTooltip(group.events)}</title><circle className={`event-dot ${kind}`} cx={x(group.seconds)} cy={row * 33 + 14} r={group.events.length > 1 ? 10 : 6} />{group.events.length > 1 && <text className="event-group-count" x={x(group.seconds)} y={row * 33 + 17.5} textAnchor="middle">{group.events.length}</text>}</g>); })()}</g>)}{tooltip && <line className="chart-cursor" x1={x(tooltip.time)} x2={x(tooltip.time)} y1="0" y2={timelineHeight - 27} />}{x.ticks(6).map(value => <g key={value}><line className="chart-grid" x1={x(value)} x2={x(value)} y1="0" y2={timelineHeight - 27} /><text className="chart-label" x={x(value)} y={timelineHeight - 8} textAnchor="middle">{formatTime(value)}</text></g>)}</svg></div><p className="report-note">Each series is independently normalized to 0–100. Rates and acceleration use exact sample intervals; no values are interpolated.</p></section>
  </>;
}
